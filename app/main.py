    ...
    <head>
        ...
        <script>
                ...
                try {
                    ...
                    container.innerHTML = logs.map(l => `
                        <div class="flex items-center justify-between py-2 border-b border-slate-800/60 text-xs font-mono">
                            ...
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold ${l.status === 'SUCCESS' || l.status === 'VERIFIED' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}">${l.status}</span>
                        </div>
                    `).join('');
                } catch (e) {
            ...
            async function triggerAction(actionName) {
                ...
                setTimeout(() => { btn.innerText = 'Done'; }, 800);
                setTimeout(() => { btn.innerText = actionName.replace('-', ' ').toUpperCase(); }, 2500);
                pollMetrics();
                pollLogs();
            }
    ...
    <body class="bg-slate-950 text-slate-100 font-sans antialiased min-h-screen flex flex-col justify-between selection:bg-indigo-500 selection:text-white">
        ...
        <header class="border-b border-slate-800 bg-slate-900/50 backdrop-blur sticky top-0 z-50">
            <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    <span class="px-3 py-1 text-xs font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full">MCP Streamable HTTP</span>
        ...
        <main class="max-w-7xl mx-auto px-6 py-8 w-full grid gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
                    ...
                    <p id="metric-leads" class="text-3xl font-extrabold text-white mt-2">1,428</p>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
                    ...
                    <p id="metric-volume" class="text-3xl font-extrabold text-white mt-2">$384,500</p>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
                    ...
                    <p id="metric-latency" class="text-3xl font-extrabold text-white mt-2">94ms</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="text-base font-semibold text-white">Ambient Voice Bridge</h2>
                            <span class="flex h-2 w-2 relative">
                              <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                              <span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                            </span>
                        </div>
                        <p class="text-xs text-slate-400 mb-4">Listening for Alexa+ Track voice-command execution hooks and intent payloads.</p>
                        <div class="bg-slate-950 border border-slate-800 rounded-lg p-3 font-mono text-[11px] text-slate-300 space-y-2 mb-4">
                            <p class="text-indigo-400 font-bold">&gt; Alexa, trigger pipeline sync</p>
                            <p class="text-slate-500">Parsing intent: <code class="text-slate-200">revops.sync.execute</code></p>
                            <p class="text-emerald-400">Response: 200 OK (Bridge Active)</p>
                        </div>
                    </div>
                    <div class="text-xs text-slate-500 border-t border-slate-800/80 pt-3">
                        Secure Webhook Token: <span class="font-mono text-slate-400">sk_alexa_99a8...</span>
                    </div>
                </div>
...
@app.get("/api/metrics")
def get_metrics():
    metrics_state["leads_enriched"] += random.choice([0, 1, 2])
    metrics_state["trust_volume"] += random.choice([0, 250, 500])
    metrics_state["mcp_latency"] = random.randint(85, 115)
    return metrics_state

@app.get("/api/friction-logs")
def get_friction_logs():
    sources = ["n8n-webhook", "supabase-db", "clay-fete", "alexa-bridge"]
    events = [
        "Lead enrichment record verified",
        "RIGS score threshold recalculated",
        "Trust settlement escrow cleared",
        "Voice command intent parsed successfully"
    ]
    statuses = ["SUCCESS", "VERIFIED", "SUCCESS"]
